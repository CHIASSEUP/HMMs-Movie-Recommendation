...
#define MaxMovies 1685                   /* Max number of movies */ 
...
int FrequencyOf_Occupations(User * TheUsers, int NbUsers, char TheOccupations[MaxOccupations][50], int FreqOfThe_Occupations[MaxOccupations]){
	int i, j, Nb_Occupations;
	
	Nb_Occupations = 0;
	for(i=0;(i < NbUsers);i++){
		j=0;
		while((j < Nb_Occupations)&&(strcmp(TheUsers[i].occupation,TheOccupations[j]) != 0)){
			j++;
		}
		if(j < Nb_Occupations){
			FreqOfThe_Occupations[j] += 1;
		}else{
			strcpy(TheOccupations[Nb_Occupations],TheUsers[i].occupation);
			FreqOfThe_Occupations[j] = 1;
			Nb_Occupations++;
		}
	}
	...
	for(i=0;(i < Nb_Occupations);i++){
		printf("%s|%i\n",TheOccupations[i],FreqOfThe_Occupations[i]);
	}
	**/
