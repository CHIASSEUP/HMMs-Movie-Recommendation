...
int main(int NbArg, char ** MesArg) {
	...
	**/
	...
	int NbNeighbours, * NbMovieRecom, ** TheNeighbours;
	int BadRecoms, MaxRecoms, NbComponents; 	
	...
	**/
	...
	if(TmaxWatchedMovies < 20){
		TmaxWatchedMovies = 20;
	}else if(TmaxWatchedMovies > MaxWatchedMovies){
		TmaxWatchedMovies = MaxWatchedMovies;
	}

	TmaxUsersWatching = atoi(MesArg[2]);
	if(TmaxUsersWatching < 20){
		TmaxUsersWatching = 20;
	}else if(TmaxUsersWatching > MaxUsersWatching){
	...
	NbOccupations = FrequencyOf_Occupations(TheUsers,NbUsers,TheOccupations,FreqOfThe_Occupations);
